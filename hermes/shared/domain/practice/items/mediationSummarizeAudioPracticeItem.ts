...
export type MediationSummarizeAudioJSON = PracticeItemJSON & {
  audioUrl: string;
...
export class MediationSummarizeAudioPracticeItem extends PracticeItem {
  ...
  constructor(json: MediationSummarizeAudioJSON) {
  ...
  override evaluate(userResponse: unknown): EvaluationResult {
    // MVP assumes transcript/summary text provided
    ...
    return {
      ...
      isCorrect: score === 1,
      conceptResults,
      feedback: "Audio summary scoring is heuristic in MVP.",
      meta: { audioUrl: this.audioUrl, wordCount: wc, withinBounds, keyPointHits: hits, keyPointTotal: kp.length },
    };
  }
}
